...
| 规则 | 说明 |
|------|------|
| **统一连续编号** | 1-50统一编号，增强章节（补贴/IP/融资/中标等）作为38-49章嵌入，**禁止**A-L字母编号 |
| **目录即正文** | TOC中的章节名称必须与正文H2标题**逐字一致**（包括括号内容） |
| **附录统一入口** | 多个附录子项（A/B/C/D）用###子标题，统一在"## 50. 附录"下 |
...
**增强章节统一编号，A-D 前置。** 对标凌度标杆：

```
目录
A. 政府补贴与政策红利分析    ← 前置深度模块（目录后、正文前）
B. 知识产权指数深度分析
C. 融资历程深度分析
D. 行业密度与竞争位势
1. 投资结论                   ← 核心报告正文
...
...
37. 退出路径与IPO可行性
38. 中标金额交叉验证          ← 末尾增强章节（正文后）
39. 专利IPC聚类
...
42. 附录
```

**铁律：A-D 四章必须在目录后、正文前。38+ 增强章节在正文末尾。** 禁止将所有增强章节统一放在末尾。
...
- 🔴 **PDF 样式保持简洁**：禁止封面页、页眉页脚、红框突出等视觉升级。使用基础表格样式（深色表头+交替行色即可）

---
description: 生成投行级企业深度研究报告。首发即深度，7篇52章结构，真实财务数据驱动。精简规则，强调内容深度高于格式检查。
...
| 检查项 | 标准 | 不合规后果 | 小微豁免 |
|--------|------|------|------|
| 目标行数 | ≥1,800行（上市）/ ≥1,500行（非上市≥1亿营收）/ ≥400行（小微企业） | 警告 | ✅ 数据不可得时降标 |
| 数据表格数 | ≥50个（上市）/ ≥30个（非上市） | 警告 | — |
| "分析："段落数 | ≥30个（上市）/ ≥20个（非上市） | 警告 | — |
| "关键发现："callout | ≥15个（上市）/ ≥10个（非上市） | 警告 | — |
| Web搜索 | ≥5次 | 禁止进入Phase 3 | 不可豁免 |
| Doris DAAS | ≥8张表（尝试） | 禁止进入Phase 3 | 不可豁免 |
| 可比交易法 | 至少2个案例 | 禁止交付 | 不可豁免 |

> 以上为**柔性引导**（非硬性阻断）。质量的核心是内容深度和可读性，非机械行数。不达标时应在补充分析中追加而非重写。
...
**搜索量要求：** 每类至少 1 次搜索，合计 ≥5 次。上市公司可追加：研报/估值对标/政策环境等搜索。
...
**⚠️ Doris 列名陷阱：** 技能文档中的列名可能与实际表结构不同。已在 `references/daas-column-mappings.md` 中记录实测列名。查询前先参考该文件，遇到 `Unknown column` 错误时执行 `DESCRIBE 表名` 查看真实列名。**特别注意：** `opt_company_label_base_10` 和 `opt_company_label_potential_01` 两张标签表仅有 `company_id` 列（无 `company_name`），必须先从 `opt_company_base` 获取 `company_id` 再查询。金融类企业的 `tb_annual_report_2` 数据可能失真（利息收入总额vs净额），优先以上市公司经审计年报为准（详见 `references/api-pitfalls.md` 陷阱9）。
...
print(f'✅ 门禁: {lines}行/{table_count}表/{analysis_count}分析/{keyfinding_count}发现')
print('⚠️ 不达标时优先追加补充分析而非拒绝交付')
```
...
主智能体直接撰写完整 MD 文件。推荐 `write_file` 一次性写入完整报告，或分2-3次写入后追加补充分析。禁止 delegate_task 写报告——子智能体丢失深度（实测958行 vs 2053行）。

**写作规则：**

### 每表必配"分析"段落

**每张数据表格后，必须跟一段 100-300 字的"分析："段落。** 要求：展开推理链、解读数据背后的含义/趋势/异常、关联行业对标和公司战略。禁止一句话点评。建议引入量化指标（CAGR/百分比变化/倍数）增强说服力。
...
**推荐方案：terminal Python heredoc + subprocess 调用 Puppeteer（🔴 2026-06-13 修正）**

由于 `execute_code` 沙箱文件写操作不持久化，PDF渲染必须通过 `terminal` 执行。使用 `python3 << 'PYEOF'` 封装：
...
<style>
  ...
  table { border-spacing: 0; width: 100%; margin: 14px 0 18px; font-size: 8pt; page-break-inside: avoid; border: 3px solid #0d2b45; }
  th { background: #0d2b45; color: #fff; padding: 8px 7px; text-align: center; font-weight: bold; border-right: 2px solid #0d2b45; border-bottom: 2px solid #0d2b45; font-size: 8pt; }
  td { padding: 5px 7px; vertical-align: top; background: #fff; border-right: 1.5px solid #5c7996; border-bottom: 1.5px solid #5c7996; font-size: 8pt; }
...
</style></head><body>BODY_PLACEHOLDER</body></html>"""

html_body = markdown.markdown(content, extensions=['tables', 'fenced_code'])
html_doc = template.replace('BODY_PLACEHOLDER', html_body)
pdf_path = base / 'report_企业名.pdf'
...
proc = subprocess.run(
    ...
    input=html_doc, capture_output=True, text=True, timeout=120
)
print(f"PDF: {'OK' if proc.returncode == 0 else 'FAIL'} | Size: {pdf_path.stat().st_size/1024/1024:.1f}MB")
PYEOF
```

**备选方案：** 如果 MD 文件较小（<30KB），可直接使用 `generate_pdf` 工具传入 HTML 字符串。

**前置条件：** `pip install markdown`（支持 `tables` 和 `fenced_code` 扩展）
...
```bash
R="report_企业名.md"
echo "行数:$(wc -l < $R) 篇:$(grep -c '^## 第.篇' $R) 分析:$(grep -c '分析：' $R) KF:$(grep -c '关键发现' $R) 日期:$(grep -c '报告日期' $R)"
```
预期：篇分组=7, 分析≥30, KF≥15, 日期=1
...
更新技能后需推送到 GitHub 技能广场。由于 `hermes skills publish` 存在扫描阻塞和认证限制，推荐使用手动 Git 流程。详见 `references/skill-publishing.md`。

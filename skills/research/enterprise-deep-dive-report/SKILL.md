---
description: 生成投行级企业深度研究报告。首发即深度（1800行以上50表格以上），Web搜索Doris DAAS可比交易法强制标配。7篇37章结构，真实财务数据驱动。
...
为企业生成投行级深度分析报告（Markdown + PDF）。**🔴 首发即深度：不再存在"先薄后厚"模式。每份报告首次交付就必须达到凌度智能报告的深度级别（1,800+行、50+表格、全维度覆盖）。**
...
| 目标行数 | ≥1,800行 | 禁止交付 |
| 数据表格数 | ≥50个 | 禁止交付 |
| Web搜索 | ≥5次（TAM/可比交易/国际对标/产业链/管理层） | 禁止进入Phase 3 |
| Doris DAAS | ≥5张表（IP指数/补贴/融资/标签/规模/行业密度） | 禁止进入Phase 3 |
| 可比交易法 | 至少2个同行业融资/并购案例 | 禁止交付 |
...
# 🔴 深度门禁（新增）
table_count = len(re.findall(r'^\|[-|]+\|$', content, re.MULTILINE)) // 2  # 每表2条分隔线
...
**写作规则：**
- 每章至少 2 个数据表格 + 1 段深度分析
- 每张表格必须有解读文字（"表格显示..."、"从上表可以看出..."）
- 财务章节（18-24章）每章至少 3 张表格
- 估值章节（33-37章）必须有 DCF 三情景 + 可比交易法 + 可比公司法三种估值
- 全文无"V4""新增""增强"等迭代痕迹
- 读起来像一次性完稿
...
**验证脚本**（见 `references/report-quality-checklist.md`）。
